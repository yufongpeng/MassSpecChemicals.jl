...
@testset "Utils" begin 
    @testset "Combinatorics" begin 
        @test MSC.safe_factorial(false, 1, 1) == 1
        @test isapprox(MassSpecChemicals.safe_factorial(false, 10, 3), factorial(10, 3))
        @test isapprox(MassSpecChemicals.safe_factorial(false, 50, 3), factorial(big(50), 3))
        @test isapprox(MassSpecChemicals.safe_multinomial(false, [50, 50]), MassSpecChemicals.safe_multinomial(false, 50, 50))
        @test MassSpecChemicals.safe_multinomial(false, 50) == 1
        @test MassSpecChemicals.safe_multinomial(50) == 1
        @test isapprox(MassSpecChemicals.safe_multinomial(false, [10, 10]), MassSpecChemicals.safe_multinomial(false, 10, 10))
    end
